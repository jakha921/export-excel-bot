...
def parse_invoice_data(file_path):
    ...
    for sheet in sheets:
        ...
        # if B15 and C15 is empty continue iteration. Example if B15 is empty C15 must be empty too
        if not sheet["B15"].value:
            print("skip sheet", sheet.title)
            continue
        ...
        try:
            ...
            # Check if the date of birth is in the correct format and accept 08,09,1992 this format also
            # if not isinstance(sheet["D8"].value, datetime) or not re.match(r'\d{2},\d{2},\d{4}',
            #                                                                str(sheet["D8"].value)):
            dob_value = str(sheet["D8"].value).split()[0].replace('-', '.').replace(',', '.')

            # Change formate 1988.12.27 to 27.12.1988
            if re.match(r'\d{4}.\d{2}.\d{2}', dob_value):
                dob_value = datetime.strptime(dob_value, "%Y.%m.%d")
                dob_value = dob_value.strftime("%d.%m.%Y")

            if not isinstance(dob_value, datetime):
                # Attempt to match either format using regular expressions
                if not re.match(r'\d{2}\.\d{2}\.\d{4}$', str(dob_value)) and not re.match(r'\d{2},\d{2},\d{4}$',
                                                                                          str(dob_value)):
                    raise ValueError(f'Incorrect date format: {dob_value}')
            ...
            error_messages.append(f'Sheet {sheet.title}: {e}')
